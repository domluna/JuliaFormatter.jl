...
function pretty(
    ...
    ret = if k == K"Identifier" && JuliaSyntax.is_leaf(node)
        p_identifier(style, node, s, ctx, lineage)
    elseif JuliaSyntax.is_operator(node) && JuliaSyntax.is_leaf(node)
    ...
    elseif is_punc(node) && JuliaSyntax.is_leaf(node)
        p_punctuation(style, node, s, ctx, lineage)
    elseif JuliaSyntax.is_keyword(node) && JuliaSyntax.is_leaf(node)
    ...
    elseif k === K"." && !JuliaSyntax.is_leaf(node)
    ...
    elseif k === K"quote" && !JuliaSyntax.is_leaf(node) && kind(node[1]) === K":"
        p_quotenode(style, node, s, ctx, lineage)
    elseif k === K"quote" && !JuliaSyntax.is_leaf(node)
...
function p_functiondef(
    ...
    for (i, c) in enumerate(childs)
        ...
        elseif kind(c) === K"block" && !JuliaSyntax.is_leaf(c)
            block_has_contents =
...
function p_module(
    ...
    for c in childs
        if kind(c) in KSet"module baremodule" && JuliaSyntax.is_leaf(c)
            n = pretty(style, c, s, ctx, lineage)
            add_node!(t, n, s; join_lines = true)
            add_node!(t, Whitespace(1), s)
        elseif kind(c) === K"end"
...
function p_const(
    ...
    for c in children(cst)
        ...
        elseif !JuliaSyntax.is_whitespace(c) && !JuliaSyntax.is_keyword(c)
            add_node!(t, Whitespace(1), s)
        elseif !JuliaSyntax.is_whitespace(c) && JuliaSyntax.is_keyword(c) && !JuliaSyntax.is_leaf(c)
            add_node!(t, Whitespace(1), s)
        end
        add_node!(t, pretty(style, c, s, ctx, lineage), s; join_lines = true)
    end
...
function p_for(
    ...
    for c in children(cst)
        if kind(c) in KSet"for while" && JuliaSyntax.is_leaf(c)
...
function p_do(
    ...
    childs = children(cst)
    for (i, c) in enumerate(childs)
        if kind(c) === K"do" && JuliaSyntax.is_leaf(c)
            add_node!(t, Whitespace(1), s)
            add_node!(t, pretty(style, c, s, ctx, lineage), s; join_lines = true)
...
function p_binaryopcall(
    ...
    for (i, c) in enumerate(childs)
        @info "" JuliaSyntax.is_operator(c) kind(c)
        n = pretty(
            style,
            c,
            s,
        ...
        is_dot = kind(c) === K"."
        if is_dot && !JuliaSyntax.is_leaf(c) && length(children(c)) == 2
            # [.]
            #   .
            #   <=
            ns = is_dot ? 1 : nws
            ...
            after_op = true
            # elseif (kind(c) === opkind || kind(c) === K".") && JuliaSyntax.is_leaf(c)
        elseif JuliaSyntax.is_operator(c) && JuliaSyntax.is_leaf(c) && i in op_indices
            ...
            if ns > 0 && i > 1
                if kind(childs[i-1]) !== K"."  # Don't add space if previous was a dot
                    add_node!(t, Whitespace(ns), s)
                elseif kind(childs[i-1]) === K"." && !JuliaSyntax.is_leaf(childs[i-1])  # Don't add space if previous was a dot
                    add_node!(t, Whitespace(ns), s)
                end
            end
...
function p_whereopcall(
    ...
    childs = children(cst)
    where_idx = findfirst(c -> kind(c) === K"where" && JuliaSyntax.is_leaf(c), childs)
    ...
    after_where = false
    for (i, a) in enumerate(childs)
        if kind(a) === K"where" && JuliaSyntax.is_leaf(a)
            add_node!(t, Whitespace(1), s)
            add_node!(t, pretty(style, a, s, ctx, lineage), s; join_lines = true)
            add_node!(t, Whitespace(1), s)
            after_where = true
...
function p_conditionalopcall(
    ...
    for c in children(cst)
        if kind(c) in KSet"? :" && JuliaSyntax.is_leaf(c)
            add_node!(t, Whitespace(1), s)
            add_node!(t, pretty(style, c, s, ctx, lineage), s; join_lines = true)
            add_node!(t, Placeholder(1), s)
        else
...
function p_call(
    ...
    for (i, a) in enumerate(childs)
        k = kind(a)
        n = if k == K"=" && !JuliaSyntax.is_leaf(a)
...
function p_invisbrackets(
    ...
    nest = if length(args) > 0
        arg = args[1]
        if is_block(arg) ||
           (kind(arg) === K"generator" && !JuliaSyntax.is_leaf(arg) && is_block(arg[1]))
            t.nest_behavior = AlwaysNest
        end
...
function p_tupleblock(
    ...
    childs = children(cst)
    for (i, a) in enumerate(childs)
        n = if kind(a) === K"=" && !JuliaSyntax.is_leaf(a)
            p_kw(style, a, s, ctx, lineage)
        else
            pretty(style, a, s, ctx, lineage)
        end
...
function p_comprehension(
    ...
    if is_block(arg)
        t.nest_behavior = AlwaysNest
    elseif kind(arg) === K"generator" && !JuliaSyntax.is_leaf(arg)
...
function p_import(
    ...
    for a in children(cst)
        if kind(a) in KSet"import export public using"
            add_node!(t, pretty(style, a, s, ctx, lineage), s; join_lines = true)
            add_node!(t, Whitespace(1), s)
        elseif kind(a) === K":" && !JuliaSyntax.is_leaf(a)
            nodes = children(a)
            for n in nodes
...
function p_generator(
    ...
    for (i, a) in enumerate(childs)
        ...
        if JuliaSyntax.is_keyword(a) && JuliaSyntax.is_leaf(a)
