...
#
# forward nesting - additional length applies only
# to the final node
#
# Curly/Call +1 i.e. ","
# MacroCall +1 i.e. "," or " "
# WhereOpCall (A part of "A where B")
#
# backward nesting
#
# BinaryOpCall + whitespace + length(op)
# ConditionalOpCall +2 i.e. " ?" " :"
#
...
nest!(x::AbstractLeaf, s::State; el=0) = (s.line_offset += length(x); nothing)

function nest!(x::PTree, s::State; el=0)
...
function nest!(x::PTree{CSTParser.EXPR{T}}, s::State; el=0) where T <: Union{CSTParser.Import,CSTParser.Using,CSTParser.Export}
...
function nest!(x::PTree{CSTParser.EXPR{T}}, s::State; el=0) where T <: Union{CSTParser.TupleH,CSTParser.Vect,CSTParser.InvisBrackets,CSTParser.Braces,CSTParser.Parameters}
    line_width = s.line_offset + length(x) + el
...
function nest!(x::PTree{CSTParser.EXPR{T}}, s::State; el=0) where T <: Union{CSTParser.Curly,CSTParser.Call}
    line_width = s.line_offset + length(x) + el
    idx = findlast(n -> is_placeholder(n), x.nodes)
    @info "ENTER" typeof(x) s.line_offset line_width el
    if idx !== nothing && line_width > s.max_width
...
function nest!(x::PTree{CSTParser.EXPR{CSTParser.MacroCall}}, s::State; el=0)
    line_width = s.line_offset + length(x) + el
    idx = findlast(n -> is_placeholder(n), x.nodes)
    #= @info "ENTER" typeof(x) s.line_offset line_width el =#
...
function nest!(x::PTree{CSTParser.WhereOpCall}, s::State; el=0)
    #= @info "ENTER" typeof(x) s.line_offset x =#
    line_width = s.line_offset + length(x) + el
    if line_width > s.max_width
        line_offset = s.line_offset
        ...
        for (i, n) in enumerate(x.nodes[idx+1:end])
            ...
            elseif is_placeholder(n) 
                ...
                i + 2 == length(Blens) && (line_width += el)
                if line_width > s.max_width
        ...
        ph_offset = max(ph_offset, s.line_offset)
        el = ph_offset - length(x.nodes[1])

        # A where
        s.line_offset = line_offset
        nest!(x.nodes[1], s, el=el)
        ...
        #= @info "WHERE DIFF" diff A_where_len s.line_offset =#
        f = (x, s) -> begin
...
# E1 :
# E2
function nest!(x::PTree{CSTParser.ConditionalOpCall}, s::State; el=0)
...
# arg1 op
# arg2
function nest!(x::PTree{T}, s::State; el=0) where T <: Union{CSTParser.BinaryOpCall,CSTParser.BinarySyntaxOpCall}
